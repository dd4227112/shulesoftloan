...
@section('card-action-btn')
    @if (Gate::check('create account type'))
        <a class="btn btn-secondary btn-sm ml-20 customModal" href="#" data-size="md"
            data-url="{{ route('account-type.create') }}" data-title="{{ __('Create Account Type') }}"> <i
                class="ti-plus mr-5"></i>{{ __('Create Account Type') }}</a>
    @endif
@endsection
@section('content')
    <div class="row">
                        ...
                        @if (Gate::check('create account type'))
                            <div class="col-auto">
                                <a class="btn btn-secondary btn-sm customModal" href="#" data-size="md"
                                    data-url="{{ route('account-type.create') }}" data-title="{{ __('Create Account Type') }}">
                                                ...
                                                <div class="cart-action">
                                                    ...
                                                    @if (Gate::check('edit account type'))
                                                        <a class="text-success customModal" data-bs-toggle="tooltip"
                                                            ...
                                                            data-url="{{ route('account-type.edit', $accountType->id) }}"
