...
@section('card-action-btn')
    @if (Gate::check('create loan type'))
        <a class="btn btn-secondary btn-sm ml-20 customModal" href="#" data-size="lg"
            data-url="{{ route('loan-type.create') }}" data-title="{{ __('Create Loan Type') }}"> <i
                class="ti-plus mr-5"></i>{{ __('Create Loan Type') }}</a>
    @endif
@endsection
@section('content')
    <div class="row">
                                        ...
                                        @if (Gate::check('edit loan type') || Gate::check('delete loan type'))
                                            <td>
                                                <div class="cart-action">
                                                    {!! Form::open(['method' => 'DELETE', 'route' => ['loan-type.destroy', $loanType->id]]) !!}
                                                    @if (Gate::check('edit loan type'))
                                                        <a class="text-success customModal" data-bs-toggle="tooltip"
                                                            data-bs-original-title="{{ __('Edit') }}" data-size="lg"
                                                            href="#"
                                                            data-url="{{ route('loan-type.edit', $loanType->id) }}"
