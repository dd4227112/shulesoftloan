...
@section('card-action-btn')
    @if (Gate::check('create transaction'))
        <a class="btn btn-secondary btn-sm ml-20 customModal" href="#" data-size="md"
            data-url="{{ route('transaction.create') }}" data-title="{{ __('Create Transaction') }}"> <i
                class="ti-plus mr-5"></i>{{ __('Create Transaction') }}</a>
    @endif
@endsection
@section('content')
    <div class="row">
                            ...
                            <tbody>
                                @foreach ($transactions as $transaction)
                                    <tr>
                                        ...
                                        <td>{{ $transaction->Customers?->name??'Deleted User' }} </td>
