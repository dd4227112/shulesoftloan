...
@section('card-action-btn')
    @if (Gate::check('create repayment'))
        <a class="btn btn-secondary btn-sm ml-20 customModal" href="#" data-size="md"
            data-url="{{ route('repayment.create') }}" data-title="{{ __('Create Repayment') }}"> <i
                class="ti-plus mr-5"></i>{{ __('Create Repayment') }}</a>
    @endif
@endsection
@section('content')
    <div class="row">
                            ...
                            <tbody>
                                @foreach ($repayments as $repayment)
                                    <tr>
                                        ...
                                        <td>{{ $repayment->Panality ? priceFormat($repayment->Panality) : 0.0 }} </td>
                                        <td>{{ priceFormat($repayment->total_amount) }} </td>
                                        @if (Gate::check('edit repayment') || Gate::check('delete repayment'))
                                            <td>
                                                <div class="cart-action">
                                                    {!! Form::open(['method' => 'DELETE', 'route' => ['repayment.destroy', $repayment->id]]) !!}
                                                    @if (Gate::check('edit repayment'))
                                                        <a class="text-success customModal" data-bs-toggle="tooltip"
                                                            data-bs-original-title="{{ __('Edit') }}" href="#"
                                                            data-url="{{ route('repayment.edit', $repayment->id) }}"
