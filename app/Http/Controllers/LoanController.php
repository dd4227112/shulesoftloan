...
class LoanController extends Controller
{
    ...
    public function index()
    {
        if (\Auth::user()->can('manage loan')) {
            $loans = Loan::where('parent_id', parentId())->get();
                ...
                foreach ($request->document_type as $key => $value) {
                    if ($value) {
                        ...
                        if (isset($request->document[$key])) {
                            ...
                            $photo = $request->document[$key]->storeAs('upload/loan_document', $name, 'public');
                            $loanDocument->document = $name;
                        }
                        $loanDocument->save();
                    }
                }
                ...
                foreach ($request->document_type as $key => $value) {
                    ...
                    if (isset($request->id[$key]) && in_array($request->id[$key], $ids)) {
                        ...
                        if ($request->document && isset($request->document[$key])) {
                            $extension = $request->document[$key]->getClientOriginalExtension();
                            $name = \Str::uuid() . '.' . $extension;
                            $photo = $request->document[$key]->storeAs('upload/loan_document', $name);
                            $loanDocument->document = $name;
                        }
                        $loanDocument->save();
