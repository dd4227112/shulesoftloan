...
class SettingController extends Controller
{
    ...
    public function index()
    {
        $user = \Auth::user();
        $settings = settings();
        $timezones = require base_path('config/timezones.php');
    ...
    public function accountData(Request $request)
    {
        ...
        if ($request->hasFile('profile')) {
            ...
            if ($user->profile) {
                Storage::delete('uploads/profile/' . $user->profile);
            }
            ...
            // Store in storage/app/uploads/profile
            $path = $request->file('profile')->storeAs('upload/profile', $fileNameToStore, 'public');
        ...
        return redirect()->back()
            ->with('success', 'User profile settings successfully updated.')
            ->with('tab', 'user_profile_settings');
    }
